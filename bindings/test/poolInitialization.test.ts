...
it("keeps BLS and configuration-only environments below the unused pool allocation", {timeout: 30_000}, () => {
  const workerSource = `
    ...
    globalThis.config = new bindings.BeaconConfig(mainnetChainConfig, new Uint8Array(32));
...
it("defers invalid capacity until state creation and retries failed pool initialization", {timeout: 30_000}, () => {
  runProcess(
    ...
    assert.throws(() => bindings.BeaconStateView.createFromBytes(bytes.subarray(0, 47), config), {code: "InvalidStateBytes"});
    for (const capacity of ["invalid", "2147483647"]) {
      process.env.LODESTAR_Z_NODE_POOL_CAPACITY = capacity;
      for (let i = 0; i < 10; i++) {
        assert.throws(() => bindings.BeaconStateView.createFromBytes(bytes, config), {code: "InvalidPoolCapacity"}, capacity + ": " + i);
      }
    }
