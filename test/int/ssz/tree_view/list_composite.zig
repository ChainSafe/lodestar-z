...
test "TreeView composite list clone isolates updates" {
    ...
    try Checkpoint.tree.toValue(v1_e0.root, &pool, &v1_e0_value);

    const v2_e0 = try v2.get(0);
    var v2_e0_value: Checkpoint.Type = undefined;
    try Checkpoint.tree.toValue(v2_e0.root, &pool, &v2_e0_value);
...
test "TreeView composite list clone drops uncommitted changes" {
    ...
    try Checkpoint.tree.toValue(v_e0_before.root, &pool, &v_e0_before_value);
    ...
    try Checkpoint.tree.toValue(v_e0_after.root, &pool, &v_e0_after_value);

    const dropped_e0 = try dropped.get(0);
    var dropped_e0_value: Checkpoint.Type = undefined;
    try Checkpoint.tree.toValue(dropped_e0.root, &pool, &dropped_e0_value);
