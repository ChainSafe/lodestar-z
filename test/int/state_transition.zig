...
const generateElectraBlindedBlock = state_transition.test_utils.generateElectraBlindedBlock;
...
const SignedBlindedBeaconBlock = state_transition.state_transition.SignedBlindedBeaconBlock;
...
test "state transition - electra blinded block" {
    const test_cases = [_]TestCase{
        .{ .transition_opt = .{ .verify_signatures = true }, .expect_error = true },
        .{ .transition_opt = .{ .verify_signatures = false, .verify_proposer = true }, .expect_error = true },
        .{ .transition_opt = .{ .verify_signatures = false, .verify_proposer = false, .verify_state_root = true }, .expect_error = true },
    ...
    inline for (test_cases) |tc| {
        ...
        var test_state = try TestCachedBeaconStateAllForks.init(allocator, 256);
        defer test_state.deinit();
        const electra_blinded_block_ptr = try allocator.create(types.electra.SignedBlindedBeaconBlock.Type);
        try generateElectraBlindedBlock(allocator, test_state.cached_state, electra_blinded_block_ptr);
        defer {
            types.electra.SignedBlindedBeaconBlock.deinit(allocator, electra_blinded_block_ptr);
            allocator.destroy(electra_blinded_block_ptr);
        }
        const signed_blinded_beacon_block = SignedBlindedBeaconBlock{ .electra = electra_blinded_block_ptr };
        const signed_block = SignedBlock{ .blinded = signed_blinded_beacon_block };
        // TODO: if blst can publish BlstError.BadEncoding, can just use testing.expectError
        const res = stateTransition(allocator, test_state.cached_state, signed_block, tc.transition_opt);
        if (tc.expect_error) {
            if (res) |_| {
