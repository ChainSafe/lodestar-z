...
```sh
__AFL_DEFER_FORKSRV=1 afl-cmin.bash \
...
without it the binary exits with code 2. Use `afl-cmin.bash` (not the
Python `afl-cmin`) to avoid wrapper bugs in some AFL++ versions.
