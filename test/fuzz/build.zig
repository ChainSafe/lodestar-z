...
pub fn build(b: *std.Build) void {
    ...
    inline for (fuzzers) |fuzzer| {
        ...
        const extra_libs: []const *std.Build.Step.Compile = if (fuzzer.needs_blst) &.{dep_blst.artifact("blst")} else &.{};
        const exe = afl.addInstrumentedExe(b, lib, extra_libs);
