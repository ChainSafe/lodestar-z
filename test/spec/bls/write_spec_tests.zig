...
pub fn main() !void {
    ...
    const out = try std.Io.Dir.cwd().createFile(std.Options.debug_io, "test/spec/bls/spec_tests.zig", .{});
    ...
    for (bls_test_forks) |fork_name| {
        ...
        const fork_tests_dir = std.Io.Dir.cwd().openDir(std.Options.debug_io, fork_tests_dir_name, .{ .iterate = true }) catch |err| switch (err) {
            error.FileNotFound => continue,
            else => return err,
        };
        var fork_tests_dir_it = fork_tests_dir.iterate();
        while (try fork_tests_dir_it.next(std.Options.debug_io)) |fork_test_entry| {
            ...
            const test_type_dir = try std.Io.Dir.cwd().openDir(std.Options.debug_io, test_type_dir_name, .{ .iterate = true });
            var test_type_dir_it = test_type_dir.iterate();
            while (try test_type_dir_it.next(std.Options.debug_io)) |test_type_entry| {
