    ...
    return struct {
        ...
        fn handleChecks(self: *Self, checks: Checks) !void {
            ...
            if (checks.head) |expected_head| {
                const result = try self.fc.updateAndGetHead(self.allocator, std.testing.io, .{ .get_canonical_head = {} });
            ...
            if (checks.get_proposer_head) |expected_root| {
                ...
                const result = try self.fc.updateAndGetHead(self.allocator, std.testing.io, .{
                    .get_proposer_head = .{
            ...
            if (checks.should_override_forkchoice_update) |expected_fcu| {
                ...
                const head_result = try self.fc.updateAndGetHead(self.allocator, std.testing.io, .{ .get_canonical_head = {} });
