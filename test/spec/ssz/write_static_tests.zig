...
pub fn main() !void {
    ...
    const out = try std.Io.Dir.cwd().createFile(std.Options.debug_io, "test/spec/ssz/static_tests.zig", .{});
    ...
    for (supported_forks) |fork| {
        ...
        const static_tests_dir = try std.Io.Dir.cwd().openDir(std.Options.debug_io, static_tests_dir_name, .{ .iterate = true });
        var static_tests_dir_it = static_tests_dir.iterate();
        while (try static_tests_dir_it.next(std.Options.debug_io)) |g_test_entry| {
            ...
            const type_name_tests_dir = try std.Io.Dir.cwd().openDir(std.Options.debug_io, type_name_tests_dir_name, .{ .iterate = true });
            var type_name_tests_dir_it = type_name_tests_dir.iterate();
            while (try type_name_tests_dir_it.next(std.Options.debug_io)) |type_name_test_entry| {
                ...
                const test_suite_dir = try std.Io.Dir.cwd().openDir(std.Options.debug_io, test_suite_dir_name, .{ .iterate = true });
                var test_suite_dir_it = test_suite_dir.iterate();
                while (try test_suite_dir_it.next(std.Options.debug_io)) |test_suite_entry| {
...
fn writeStaticTest(
    ...
    try writer.print(
        ...
        \\
        \\    var test_dir = std.Io.Dir.cwd().openDir(std.testing.io, test_dir_name, .{{}}) catch return error.SkipZigTest;
