...
pub fn main() !void {
    ...
    const out = try std.Io.Dir.cwd().createFile(std.Options.debug_io, "test/spec/ssz/generic_tests.zig", .{});
    ...
    const generic_tests_dir = try std.Io.Dir.cwd().openDir(std.Options.debug_io, generic_tests_dir_name, .{ .iterate = true });
    var generic_tests_dir_it = generic_tests_dir.iterate();
    outer: while (try generic_tests_dir_it.next(std.Options.debug_io)) |g_test_entry| {
        ...
        const valid_tests_dir = try std.Io.Dir.cwd().openDir(std.Options.debug_io, valid_tests_dir_name, .{ .iterate = true });
        var valid_tests_dir_it = valid_tests_dir.iterate();
        while (try valid_tests_dir_it.next(std.Options.debug_io)) |valid_test_entry| {
        ...
        const invalid_tests_dir = try std.Io.Dir.cwd().openDir(std.Options.debug_io, invalid_tests_dir_name, .{ .iterate = true });
        var invalid_tests_dir_it = invalid_tests_dir.iterate();
        while (try invalid_tests_dir_it.next(std.Options.debug_io)) |invalid_test_entry| {
...
fn writeValidTest(
    ...
    try writer.print(
        ...
        \\
        \\    const test_dir = try std.Io.Dir.cwd().openDir(std.testing.io, test_dir_name, .{{}});
