...
pub const ValidatorMetrics = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) !ValidatorMetrics {
        ...
        return .{
            .remote_sign_errors_total = Counter(u64).init(
                "vc_remote_sign_errors_total",
                .{ .help = "Total remote signer request errors." },
                ro,
            ),
            .sign_errors_total = Counter(u64).init(
                "vc_sign_errors_total",
                .{ .help = "Total validator signing errors." },
                ro,
            ),
            .slashing_protection_block_errors_total = Counter(u64).init(
                "vc_slashing_protection_block_errors_total",
                .{ .help = "Total slashing protection errors while recording block proposals." },
                ro,
            ),
            .slashing_protection_attestation_errors_total = Counter(u64).init(
                "vc_slashing_protection_attestation_errors_total",
                .{ .help = "Total slashing protection errors while recording attestations." },
                ro,
            ),
            .attestation_published_total = Counter(u64).init(
                "validator_attestation_published_total",
                .{ .help = "Total attestations published by the validator client." },
                ro,
            ),
            .attestation_missed_total = Counter(u64).init(
                "validator_attestation_missed_total",
                .{ .help = "Total attestation duties missed by the validator client." },
                ro,
            ),
            .attestation_delay_seconds = Histogram(f64, &attestation_delay_buckets).init(
                "validator_attestation_delay_seconds",
                .{ .help = "Distribution of attestation publication delay in seconds." },
                ro,
            ),
            .attestation_delay_count = Histogram(f64, &attestation_delay_buckets).init(
                "beacon_attestation_delay_count",
                .{ .help = "Compatibility histogram mirroring attestation publication delay in seconds." },
                ro,
            ),
            .attestations_published_total = Gauge(u64).init(
                "beacon_attestations_published_total",
                .{ .help = "Compatibility gauge tracking total attestations published." },
                ro,
            ),
            .attester_duties_count = Gauge(u64).init(
                "vc_attester_duties_count",
                .{ .help = "Current number of cached attester duties." },
                ro,
            ),
            .attester_duties_epoch_count = Gauge(u64).init(
                "vc_attester_duties_epoch_count",
                .{ .help = "Current number of epochs represented in the attester duty cache." },
                ro,
            ),
            .attestation_duties_reorg_total = Counter(u64).init(
                "vc_attestation_duties_reorg_total",
                .{ .help = "Total attester duty cache invalidations caused by chain reorgs." },
                ro,
            ),
            .attestation_duty_slot = Gauge(u64).init(
                "vc_attestation_duty_slot",
                .{ .help = "Next attestation duty slot in the attester duty cache, or 0 if none is cached." },
                ro,
            ),
            .block_proposed_total = Counter(u64).init(
                "validator_block_proposed_total",
                .{ .help = "Total blocks proposed by the validator client." },
                ro,
            ),
            .block_missed_total = Counter(u64).init(
                "validator_block_missed_total",
                .{ .help = "Total block proposal duties missed by the validator client." },
                ro,
            ),
            .block_delay_seconds = Histogram(f64, &block_delay_buckets).init(
                "validator_block_delay_seconds",
                .{ .help = "Distribution of block proposal delay in seconds." },
                ro,
            ),
            .block_delay_count = Histogram(f64, &block_delay_buckets).init(
                "beacon_block_delay_count",
                .{ .help = "Compatibility histogram mirroring block proposal delay in seconds." },
                ro,
            ),
            .beacon_block_proposed_total = Gauge(u64).init(
                "beacon_block_proposed_total",
                .{ .help = "Compatibility gauge tracking total blocks proposed." },
                ro,
            ),
            .proposer_duties_epoch_count = Gauge(u64).init(
                "vc_proposer_duties_epoch_count",
                .{ .help = "Current number of epochs represented in the proposer duty cache." },
                ro,
            ),
            .proposer_duties_reorg_total = Counter(u64).init(
                "vc_proposer_duties_reorg_total",
                .{ .help = "Total proposer duty cache invalidations caused by chain reorgs." },
                ro,
            ),
            .new_proposal_duties_detected_total = Counter(u64).init(
                "vc_new_proposal_duties_detected_total",
                .{ .help = "Total newly detected proposal duties." },
                ro,
            ),
            .sync_committee_message_total = Counter(u64).init(
                "validator_sync_committee_message_total",
                .{ .help = "Total sync committee messages published by the validator client." },
                ro,
            ),
            .sync_committee_contribution_total = Counter(u64).init(
                "validator_sync_committee_contribution_total",
                .{ .help = "Total sync committee contributions published by the validator client." },
                ro,
            ),
            .sync_committee_duties_count = Gauge(u64).init(
                "vc_sync_committee_duties_count",
                .{ .help = "Current number of cached sync committee duties." },
                ro,
            ),
            .sync_committee_duties_epoch_count = Gauge(u64).init(
                "vc_sync_committee_duties_epoch_count",
                .{ .help = "Current number of epochs represented in the sync committee duty cache." },
                ro,
            ),
            .sync_committee_duties_reorg_total = Counter(u64).init(
                "vc_sync_committee_duties_reorg_total",
                .{ .help = "Total sync committee duty cache invalidations caused by chain reorgs." },
                ro,
            ),
            .total_validators = Gauge(u64).init(
                "validator_total_count",
                .{ .help = "Current number of validators tracked by the validator client." },
                ro,
            ),
            .active_validators = Gauge(u64).init(
                "validator_active_count",
                .{ .help = "Current number of active validators tracked by the validator client." },
                ro,
            ),
            .local_validator_count = Gauge(u64).init(
                "local_validator_count",
                .{ .help = "Compatibility gauge currently mirroring the total validator count." },
                ro,
            ),
            .beacon_health = Gauge(u64).init(
                "vc_beacon_health",
                .{ .help = "Beacon node health state as an enum: ready=0, syncing=1, err=2." },
                ro,
            ),
            .rest_api_request_seconds = try HistogramVec(f64, RestApiRouteLabels, &rest_api_request_buckets).init(
                ...
                .{ .help = "Distribution of validator REST API client request durations in seconds by route." },
                ro,
            ),
            .rest_api_stream_seconds = try HistogramVec(f64, RestApiRouteLabels, &rest_api_request_buckets).init(
                ...
                .{ .help = "Distribution of validator REST API client stream durations in seconds by route." },
                ro,
            ),
            .rest_api_request_errors_total = try CounterVec(u64, RestApiRouteUrlLabels).init(
                allocator,
                "vc_rest_api_client_request_errors_total",
                .{ .help = "Total validator REST API client request errors by route and base URL." },
                ro,
            ),
            ...
            .keymanager_response_seconds = try HistogramVec(f64, OperationLabels, &keymanager_response_buckets).init(
                ...
                .{ .help = "Distribution of keymanager API request durations in seconds by operation." },
                ro,
            ),
            .keymanager_active_connections = Gauge(u64).init(
                "validator_keymanager_active_connections",
                .{ .help = "Current number of active keymanager API connections." },
                ro,
            ),
            .monitoring_collect_data_seconds = Histogram(f64, &monitoring_collect_buckets).init(
                "lodestar_monitoring_collect_data_seconds",
                .{ .help = "Distribution of validator monitoring data collection durations in seconds." },
                ro,
            ),
