...
pub const Protocol = struct {
    ...
    fn handleWhoareyou(
        ...
        const idx = pending_idx orelse {
            std.log.warn("discv5: WHOAREYOU for unknown nonce (pending={d}, nonce={s}, from={any})", .{
                self.pending_requests.items.len,
                &std.fmt.bytesToHex(request_nonce, .lower),
                from,
            });
            if (self.pending_requests.items.len > 0) {
                std.log.warn("discv5:   first pending nonce={s}, dest={any}", .{
        ...
        // Extract id_nonce from authdata (enr_seq at [16..24] unused — we always include ENR).
        _ = authdata[16..24]; // enr_seq field
