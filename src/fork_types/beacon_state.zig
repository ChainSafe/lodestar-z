...
pub fn BeaconState(comptime f: ForkSeq) type {
    return struct {
        ...
        pub fn validatorsAlloc(self: *Self, allocator: std.mem.Allocator) ![]ForkTypes(f).Validator.Type {
            var validators_view = try self.inner.getReadonly("validators");
            try validators_view.commit();
            return validators_view.getAllReadonlyValues(allocator);
        }

        /// Like `validatorsAlloc` but returns a slice of pointers into the
        /// pool-resident `Validator` values. ~16× lower memory than
        /// `validatorsAlloc` (8 B/elem vs 121 B/elem) and zero per-element
        /// memcpy.
        ///
        ...
        pub fn validatorsPtrsAlloc(self: *Self, allocator: std.mem.Allocator) ![]*const ForkTypes(f).Validator.Type {
            var validators_view = try self.inner.getReadonly("validators");
            try validators_view.commit();
        ...
        pub fn balancesAlloc(self: *Self, allocator: std.mem.Allocator) ![]u64 {
        ...
        pub fn proposerLookaheadAlloc(self: *Self, allocator: std.mem.Allocator) !*[ForkTypes(.fulu).ProposerLookahead.length]u64 {
