...
fn InnerShuffleList(comptime T: type, out: []T, seed: []const u8, rounds: u8, forwards: bool) !void {
    if (rounds == 0) {
        // no shuffling rounds
        return;
    }
    ...
    const ShufflingManager = struct {
        ...
        pub fn init(_seed: []const u8) !Self {
            if (_seed.len != SEED_SIZE) {
                return error.InvalidSeedLen;
            }
            var buf = [_]u8{0} ** TOTAL_SIZE;
            @memcpy(buf[0..SEED_SIZE], _seed);
            return .{ .buf = buf };
        }
