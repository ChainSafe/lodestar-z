...
pub fn generateElectraBlock(allocator: Allocator, cached_state: *const CachedBeaconStateAllForks, out: *types.electra.SignedBeaconBlock.Type) !void {
    ...
    var body = types.electra.BeaconBlockBody.default_value;
    body.attestations = attestations;
    body.execution_payload = execution_payload;

    var message = types.electra.BeaconBlock.default_value;
...
/// Generate a valid electra blinded block for the given pre-state.
pub fn generateElectraBlindedBlock(allocator: Allocator, cached_state: *const CachedBeaconStateAllForks, out: *types.electra.SignedBlindedBeaconBlock.Type) !void {
    ...
    var withdrawals_root: Root = undefined;
    try types.capella.Withdrawals.hashTreeRoot(allocator, &withdrawals_result.withdrawals, &withdrawals_root);

    var execution_payload_header = types.electra.ExecutionPayloadHeader.default_value;
