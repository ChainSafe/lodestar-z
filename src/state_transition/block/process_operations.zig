    ...
    switch (attester_slashings) {
        .phase0 => |attester_slashings_phase0| {
            for (attester_slashings_phase0) |*attester_slashing| {
                try processAttesterSlashing(ct.phase0.AttesterSlashing.Type, cached_state, attester_slashing, opts.verify_signature);
            }
        },
        .electra => |attester_slashings_electra| {
            for (attester_slashings_electra) |*attester_slashing| {
                try processAttesterSlashing(ct.electra.AttesterSlashing.Type, cached_state, attester_slashing, opts.verify_signature);
            }
        },
    }
