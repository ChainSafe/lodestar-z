...
pub fn processSyncAggregate(
    ...
    if (verify_signatures) {
        var participant_indices_storage: [preset.SYNC_COMMITTEE_SIZE]ValidatorIndex = undefined;
        const participant_indices = try sync_committee_bits.intersectValuesInto(
            ValidatorIndex,
            &participant_indices_storage,
            committee_indices,
        );
        ...
        if (participant_indices.len > 0) {
            ...
            var pubkeys_storage: [preset.SYNC_COMMITTEE_SIZE]bls.PublicKey = undefined;
            const pubkeys = pubkeys_storage[0..participant_indices.len];
...
pub fn getSyncCommitteeSignatureSet(
    ...
    var computed_participant_indices_storage: [preset.SYNC_COMMITTEE_SIZE]ValidatorIndex = undefined;
    const participant_indices_: []const ValidatorIndex = if (participant_indices) |indices|
        indices
    else blk: {
        const committee_indices = @as(*const [preset.SYNC_COMMITTEE_SIZE]ValidatorIndex, @ptrCast(try epoch_cache.current_sync_committee_indexed.get().getValidatorIndices()));
        break :blk try sync_aggregate.sync_committee_bits.intersectValuesInto(
            ValidatorIndex,
            &computed_participant_indices_storage,
            committee_indices,
        );
    };
