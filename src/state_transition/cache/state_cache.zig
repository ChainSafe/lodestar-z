...
pub const CachedBeaconState = struct {
    ...
    pub fn loadOtherState(self: *CachedBeaconState, allocator: Allocator, io: std.Io, config: *const BeaconConfig, state_bytes: []const u8, seed_validators_bytes: ?[]const u8) !*CachedBeaconState {
        ...
        const immutable = EpochCacheImmutableData{
            .config = config,
            .pubkey_cache = self.epoch_cache.pubkey_cache,
        };

        const new_cached = try createCachedBeaconState(allocator, io, any_state, immutable, .{
