...
pub const LockFreePubkeyCache = struct {
    ...
    /// Get the compressed pubkey bytes for a validator index. Lock-free. The
    /// returned pointer is stable for the cache's lifetime: published entries
    /// are immutable and chunk storage never moves.
    pub fn getPubkeyBytes(
        ...
        _ = io;
        if (index >= self.len.load(.acquire)) return null;
        return &self.entryAt(@intCast(index)).compressed;
    }
