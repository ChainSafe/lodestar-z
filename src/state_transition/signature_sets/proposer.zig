...
pub fn getBlockHeaderProposerSignatureSet(cached_state: *const CachedBeaconStateAllForks, signed_block_header: *const ct.phase0.SignedBeaconBlockHeader.Type) SingleSignatureSet {
    const config = cached_state.config;
    const state = cached_state.state;
    const epoch_cache = cached_state.getEpochCache();
    ...
    var signing_root: Root = undefined;
    try computeSigningRoot(ct.phase0.SignedBeaconBlockHeader, signed_block_header, domain, &signing_root);
