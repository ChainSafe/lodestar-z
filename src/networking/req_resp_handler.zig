...
pub const ReqRespContext = struct {
    ...
    onPeerStatus: *const fn (ptr: *anyopaque, peer_id: ?[]const u8, status: StatusMessage.Type) void,
};
...
pub fn serveRequest(
    ...
    return switch (method) {
        .status => serveStatus(request_bytes, request_meta, context, writer),
        .goodbye => serveGoodbye(request_bytes, request_meta, context, writer),
        .ping => servePing(request_bytes, context, writer),
        .metadata => serveMetadata(request_bytes, context, writer),
...
pub fn handleRequest(
    ...
    try serveRequest(allocator, method, request_bytes, .{}, context, &writer);
...
fn serveStatus(
    ...
    context.onPeerStatus(context.ptr, request_meta.peer_id, peer_status);
...
fn serveBeaconBlocksByRange(
    ...
    if (request.count > max_request_blocks) {
        return writer.writeError(.invalid_request, "Count exceeds MAX_REQUEST_BLOCKS");
    }
...
fn serveBlobSidecarsByRange(
    ...
    if (request.count > max_request_blob_sidecars) {
        return writer.writeError(.invalid_request, "Count exceeds MAX_REQUEST_BLOB_SIDECARS");
    }
...
const MockContext = struct {
    ...
    fn onPeerStatus(_: *anyopaque, peer_id: ?[]const u8, status: StatusMessage.Type) void {
...
fn collectRequest(
    ...
    errdefer collector.deinit();
    var writer = collector.writer();
    try serveRequest(allocator, method, request_bytes, .{ .peer_id = peer_id }, &MockContext.req_resp_context, &writer);
    return try collector.finish();
}
