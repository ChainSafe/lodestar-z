...
withdrawals root 校验。`stateTransition` 在真实调用中创建局部 Diagnostics，区块处理失败且有详情时
输出包含 slot 和详情的 warning，然后返回原错误。原有错误码、校验顺序和 binding 接口保持不变。

```zig
var diagnostics: st.Diagnostics = .{};
const result = st.stateTransition(allocator, io, state, block, .{
...
调用方处理 result 中的错误时，确认两个 tag 后可读取 `diagnostics.detail.?.state_transition.withdrawals_root_mismatch`
...
然后返回原来的 `error.WithdrawalsRootMismatch`。省略 `TransitionOpts.diagnostics` 时使用本次
调用的局部实例；显式传入时使用调用方的实例，便于其读取结构化详情。
直接调用 `processBlock` 或 `processWithdrawals` 时仍由调用方决定是否传入诊断对象。
