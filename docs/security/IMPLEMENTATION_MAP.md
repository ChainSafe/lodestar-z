...
| --- | --- |
| N-API exports and shared addon lifecycle | [`build.zig`](../../build.zig) and [`bindings/napi/root.zig`](../../bindings/napi/root.zig) give zapi class exports a Zig package and addon-specific identity. The identity's version component comes from `build.zig.zon`, which intentionally remains `0.0.0` independently of the npm bindings version. The root module registers exports and initializes or tears down the process-wide BLS pool and pubkey cache on first or last environment. Each environment owns its default configuration snapshot, metrics, and epoch scratch cache. It allocates its tree pool lazily on first native state construction; BLS and configuration-only environments do not reserve a pool. Cleanup releases only initialized resources. |
...
| Slot-duration configuration | [`config_snapshot.zig`](../../bindings/napi/config_snapshot.zig) treats supplied `SLOT_DURATION_MS` as authoritative and derives native `SECONDS_PER_SLOT` from it. Legacy seconds-only configuration remains accepted. Supported pre-Gloas transitions require a positive duration in whole seconds. |
...
| Sync committee duty caches | [`BeaconStateView`](../../bindings/napi/BeaconStateView.zig) returns the same cache shape for current, epoch, and slot lookups: a copied `Uint32Array` of validator indices and a `Map<number, number[]>` containing every committee position for each validator. Selection delegates to the native epoch cache, including its slot-plus-one duty offset and unavailable-period errors. |
| Persistent Merkle tree pool | [`Node.Pool`](../../src/persistent_merkle_tree/Node.zig) allocates a fixed number of user slots and returns `PoolExhausted` without resizing. Each N-API environment has a separate pool because node refcounts, free-list mutations, and hashing scratch are not synchronized. First native state construction reads `LODESTAR_Z_NODE_POOL_CAPACITY`, defaults to 10,000,000 slots per environment, and rejects invalid values. Byte-length and fork validation precede allocation; failed pool initialization leaves the environment without a pool and permits retry. View-held pool references keep late finalizers safe after environment cleanup. |
| Native state memory pressure | [`pool.zig`](../../bindings/napi/pool.zig) owns each environment's tree pool, allocator byte counter, and V8 external-memory ledger. State operations and finalizers report live node-column bytes plus metered tree payload, state-view, and epoch-cache allocations. Shared nodes and caches are counted once. The pool maintains live occupancy in constant time, including allocation rollback. The owner stores its N-API environment and disables notifications at cleanup before late finalizers release retained views. Notifications inform V8's GC policy; hosts must still size the bounded pool for retained states and collection headroom. |
| Explicit state release | [`BeaconStateView.release`](../../bindings/napi/BeaconStateView.zig) shares the idempotent finalizer cleanup path. It immediately blocks subsequent state access with `InvalidState`. Every native state operation retains the view through its deferred cleanup; release from a JavaScript input getter or output callback postpones resource destruction until the last active call returns. Cleanup releases the cached state, configuration snapshot, and pool references, reporting memory changes while the owner remains alive. Independently retained descendants remain valid. Finalization remains a fallback for unreleased views and safely ignores already released resources. |
| Reused epoch cache | [`epoch_transition_cache.zig`](../../src/state_transition/cache/epoch_transition_cache.zig) stores thread-local arrays borrowed by an `EpochTransitionCache`. Fork-specific columns grow only in their consuming fork branches. Slashing penalties use validator-indexed storage up to the last index due for penalties, or an empty slice when none are due. Capacity is retained across calls. Penalty calculation uses the total initialized by `EpochCache` and maintained by validator slashing and epoch resets. Synchronous transitions within one thread must not overlap. Separate N-API workers use independent arrays; each environment releases its scratch during cleanup. |
...
| Packed SSZ boolean tree decoding | [`FixedListType`](../../src/ssz/type/list.zig) and [`FixedVectorType`](../../src/ssz/type/vector.zig) validate canonical boolean bytes before allocating plain or chunked tree nodes. [`FixedProgressiveListType`](../../src/ssz/type/progressive_list.zig) delegates tree decoding to its validated value decoder. |
