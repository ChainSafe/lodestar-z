...
| Sync-committee cache rotation | [`EpochCache.rotateSyncCommitteeIndexed`](../../src/state_transition/cache/epoch_cache.zig) builds the replacement cache and its RC before transferring current/next references. Allocation failure preserves the shared references; RC allocation failure releases the completed raw cache. |
