...
| Persistent Merkle tree pool | [`Node.Pool`](../../src/persistent_merkle_tree/Node.zig) allocates a fixed number of user slots and returns `PoolExhausted` without resizing. The shared addon reads `LODESTAR_Z_NODE_POOL_CAPACITY` during initialization, defaults to 10,000,000 slots, and rejects invalid values. [`BeaconStateView.release`](../../bindings/napi/BeaconStateView.zig) releases a view's tree nodes and pool reference deterministically; the idempotent N-API finalizer remains a fallback. Chunked-leaf and container payloads use a separate dynamic allocator. |
| Next-epoch proposer cache | [`EpochCache`](../../src/state_transition/cache/epoch_cache.zig) owns the deferred seed or computed indexes inline; cloning copies that value independently. Pre-Fulu duty queries populate the cache only after sampling succeeds, and epoch advancement replaces it. Fulu and later use state proposer lookahead. |
| Reused epoch cache | [`epoch_transition_cache.zig`](../../src/state_transition/cache/epoch_transition_cache.zig) stores process-global arrays borrowed by an `EpochTransitionCache`. Fork-specific columns grow only in their consuming fork branches. Slashing penalties use compact storage aligned with `indices_to_slash`, or an empty slice when none are due. Capacity is retained across calls. Penalty calculation uses the total initialized by `EpochCache` and maintained by validator slashing and epoch resets. The lock covers acquisition and resize, not the full borrowed lifetime. Current safe use requires non-overlapping transitions and no concurrent teardown. |
...
| SSZ list hashing workspace | [`FixedListType`](../../src/ssz/type/list.zig) and `VariableListType` hash values and serialized elements through [`MerkleAccumulator`](../../src/hashing/merkle_accumulator.zig), retaining at most 64 chunks and one root per supported tree level. List bounds are checked before accumulation; child hashers retain their own allocation contracts. Variable-element byte hashing validates each offset before consuming the corresponding element. |
...
| Compact-proof generation workspace | [`createCompactMultiProof`](../../src/persistent_merkle_tree/proof.zig) validates descriptor shape and depth, then traverses with a `max_depth`-bounded pending-node stack. Its output grows only as traversal reaches witnesses; descriptors passed through `BeaconStateView.createMultiProof` cannot reserve their full declared output before navigation. Output capacity is proportional to witnesses already visited. Opaque-node materialization retains its temporary subtree and allocator requirements until generation completes. |
