...
| Zig error details | [`Diagnostics`](../../src/diagnostics.zig) holds one optional typed detail in caller-owned memory. The withdrawals factory copies expected/actual roots before returning the original error. The values survive candidate cleanup; callers initialize the object before each operation. No shared diagnostic state or binding conversion is introduced. |
