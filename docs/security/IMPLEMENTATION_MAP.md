...
| BitList hashing workspace | [`BitListType`](../../src/ssz/type/bit_list.zig) bounds value and serialized hashing scratch with `MerkleAccumulator`. Value hashing requires native callers to preserve the typed value's limit and storage-length invariants, checked with assertions. Serialized hashing validates the SSZ delimiter and limit before accumulation, removes the delimiter, and mixes in the bit length. |
