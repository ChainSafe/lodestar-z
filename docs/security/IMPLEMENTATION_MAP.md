...
| Progressive tree serialization workspace | [`FixedProgressiveListType.tree.serializeIntoBytes`](../../src/ssz/type/progressive_list.zig) checks serialized-size arithmetic and output capacity, then uses a bounded progressive node iterator. Exhausting the iterator validates the right-spine terminator. The list does not materialize a temporary value; child serializers retain their own workspace contracts. |
