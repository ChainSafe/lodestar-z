...
| Packed SSZ tree mutation | [`TreeViewState.editChunkedLeaf`](../../src/ssz/tree_view/utils/tree_view_state.zig) copies shared payloads or materializes sparse leaves and registers pending nodes before invoking a write. [`Node.Id.editChunkedLeaf`](../../src/persistent_merkle_tree/Node.zig) requires exclusive ownership and preserves or grows the valid length, then invalidates the cached hash. The compile-time packing callback borrows one chunk only for the call and must not access the pool. |
