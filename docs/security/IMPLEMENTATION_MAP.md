...
| Zig error details | [`Diagnostics`](../../src/diagnostics.zig) holds one optional typed detail. `stateTransition` uses a supplied target or creates a local instance for `processBlock`; on failure it warns with the slot and captured detail, then returns the original error. The withdrawals factory copies expected/actual roots, so values survive candidate cleanup. Callers initialize supplied targets before each operation. No shared diagnostic state or binding conversion is introduced. |
