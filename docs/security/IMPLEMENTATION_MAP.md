...
| Compact-proof generation workspace | [`createCompactMultiProof`](../../src/persistent_merkle_tree/proof.zig) validates the descriptor before allocating one output array, then traverses with a `max_depth`-bounded pending-node stack. Opaque-node materialization retains its temporary subtree and allocator requirements until generation completes. |
