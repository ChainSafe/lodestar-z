...
| Reused epoch cache | [`epoch_transition_cache.zig`](../../src/state_transition/cache/epoch_transition_cache.zig) defines explicitly owned reusable arrays. Core callers pass their cache explicitly. [`BeaconStateView.zig`](../../bindings/napi/BeaconStateView.zig) owns one cache per N-API environment through instance data, and its typed finalizer releases that environment's arrays without accessing JavaScript or process-global state. |
