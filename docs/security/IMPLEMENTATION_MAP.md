...
| Reused epoch cache | [`epoch_transition_cache.zig`](../../src/state_transition/cache/epoch_transition_cache.zig) defines explicitly owned reusable arrays. Core callers pass their cache explicitly. [`environment_state.zig`](../../bindings/napi/environment_state.zig) owns one optional cache per N-API environment through instance data, and its typed finalizer releases that environment's arrays without accessing JavaScript or process-global state. |
