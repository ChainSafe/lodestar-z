...
| Progressive Merkleization workspace | [`progressive.merkleizeChunks`](../../src/ssz/type/progressive.zig) and its compile-time-size wrapper accumulate each progressively sized subtree with bounded scratch and fold a bounded root array along the right spine. Hashing callers still own their input chunk buffers and child allocation contracts. |
