...
| Packed bit values | [`bit_array`](../../src/bit_array/root.zig) owns the packed value representation. `BitList` enforces its optional compile-time limit and rejects indexes whose successor cannot fit in `usize`; failed growth preserves the existing value. Native callers writing storage directly must preserve the logical byte length and zero trailing bits. SSZ bit schemas retain canonical encoding and length validation at deserialization. |
